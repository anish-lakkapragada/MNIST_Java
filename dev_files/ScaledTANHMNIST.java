...
public class ScaledTANHMNIST  {
    ...
    public static int numData = 1000; //forward and back pass would hapepn quick iwth 500, 100, 32, 16, 50, 0.0004
    ...
    public static double learningRate = 0.0000000005; //working 0.0001 with 200, 4, 2, 100, 1000 config
    public static double biasLearningRate = 0.0000002 ;
    ...
    public static ArrayList<ArrayList<Double>> reLU(ArrayList<ArrayList<Double>> x) {
        ...
        for (int i = 0; i < x.size(); i++) {
            ...
            for (int j = 0; j < x.get(i).size(); j++) {
                double num = Math.exp(x.get(i).get(j)) - Math.exp(-1 * x.get(i).get(j));
                double denum = Math.exp(x.get(i).get(j)) + Math.exp(-1 * x.get(i).get(j));
                current.add(num/denum);
            }
            newX.add(current);
        }
        return newX;
    ...
    public static double tanh(double x) {
        double num = Math.exp(x) - Math.exp(-1 * x);
        double denum = Math.exp(x) + Math.exp(-1 * x);
        return num/denum;
    }
    ...
    public static HashMap<String, Object> backPropagation(ArrayList<ArrayList<Double>> outputs, ArrayList<ArrayList<Double>> labels, ArrayList<ArrayList<Double>> rawOutputs, ArrayList<ArrayList<Double>> activation2, ArrayList<ArrayList<Double>> activation1) throws Exception {
        ...
        for (int row = 0; row < activation2.size(); row++) {
            ...
            for (int col = 0; col < activation2.get(row).size(); col++) {
                currentDeriv.add(1 - Math.pow(tanh(activation2.get(row).get(col)), 2));
            }
        ...
        for (int row = 0; row < activation1.size(); row++) {
            ...
            for (int col = 0; col < activation1.get(row).size(); col++) {
                currentDeriv.add(1 - Math.pow(tanh(activation1.get(row).get(col)), 2));
            }
    ...
    public static class DimensionalityReduction extends ScaledTANHMNIST {
