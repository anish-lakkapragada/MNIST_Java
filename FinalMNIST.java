...
public class FinalMNIST {
    ...
    public static int numData = 10000;
    public static int numVal = 500;
    public static int miniBatchSize = 10000;
    ...
    public static void adam(ArrayList<ArrayList<Double>> gradients1, ArrayList<ArrayList<Double>> gradients2,
        ...
        for (int feature = 0; feature < adamM2.size(); feature++) {
            adamM2.set(feature, scalarDivide(adamM2.get(feature), (1 - Math.pow(beta1, (epoch + 1)))));
        }
        ...
        for (int feature = 0; feature < adamV2.size(); feature++) {
            adamV2.set(feature, scalarDivide(adamV2.get(feature), (1 - Math.pow(beta2, (epoch + 1)))));
        }
    ...
    public static void saveWeights() throws Exception {
        FileOutputStream fos = new FileOutputStream("weights1_10k_GDLOCAL4_8.tmp");
        ObjectOutputStream oos = new ObjectOutputStream(fos);
        oos.writeObject(weights1);

        fos = new FileOutputStream("weights2_10k_GDLOCAL4_8.tmp");
        oos = new ObjectOutputStream(fos);
        oos.writeObject(weights2);

        fos = new FileOutputStream("weights3_10k_GDLOCAL4_8.tmp");
        oos = new ObjectOutputStream(fos);
        oos.writeObject(weights3);

        fos = new FileOutputStream("biases1_10k_GDLOCAL4_8.tmp");
        oos = new ObjectOutputStream(fos);
        oos.writeObject(biases1);

        fos = new FileOutputStream("biases2_10k_GDLOCAL4_8.tmp");
        oos = new ObjectOutputStream(fos);
