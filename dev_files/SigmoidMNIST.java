...
public class SigmoidMNIST  {
    ...
    public static void weightInit() {
        ...
        for (int feature = 0; feature < x_train.get(0).size(); feature++) {
            ...
            for (int n1 = 0; n1 < numNeurons1;n1++) {
                ...
                featureToLayer.add(rnd.nextGaussian() * Math.sqrt(1/ (double) ((784 + numNeurons1)/2)));
                v1.add(0.0);
            }
        ...
        rnd = new Random(2190201);
        for (int feature = 0; feature < numNeurons1; feature++) {
            ArrayList<Double> featureToLayer = new ArrayList<Double>();
            ArrayList<Double> v2 = new ArrayList<Double>();
            for (int n1 = 0; n1 < numNeurons2;n1++) {
                featureToLayer.add(rnd.nextGaussian() * Math.sqrt(1/(double) (numNeurons1 +numNeurons2)/2)); //* Math.sqrt(2/(numNeurons1 + numNeurons2))); //todo weight init
                v2.add(0.0);
            }
            weights2.add(featureToLayer);
            System.out.println("W2 VAR : " + DimensionalityReduction.variance(featureToLayer));
            velocity2.add(v2);
        }

        rnd = new Random(122121);
        for (int feature = 0; feature < numNeurons2; feature++) {
            ArrayList<Double> featureToLayer = new ArrayList<Double>();
            ArrayList<Double> v2 = new ArrayList<Double>();
            for (int n1 = 0; n1 < numNeurons3;n1++) {
                featureToLayer.add(rnd.nextGaussian() * Math.sqrt(1/(double) (numNeurons2 + numNeurons3)/2)); //* Math.sqrt(2/(numNeurons3 + numNeurons2))); //todo weight init
                v2.add(0.0);
            }
    ...
    public static class DimensionalityReduction extends SigmoidMNIST {
